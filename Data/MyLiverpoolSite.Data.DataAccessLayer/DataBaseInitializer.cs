    ...
    public class DatabaseInitializer : CreateDatabaseIfNotExists<LiverpoolContext> //todo WARNING ALARM
    {
        ...
        protected override void Seed(LiverpoolContext context)
        {
            if (!context.Users.Any())
            {
                ...
                InitializeBlogCategories(context);
                InitializeBlog(context);
                InitializeBlogComments(context);
        ...
        private void InitializeForumThemes(LiverpoolContext context)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                var userId = new Random().Next(1, CountUsers);
...
<img src= 'http://s3.hostingkartinok.com/uploads/images/2013/07/fc66688ba1ecc06dfe20bc6688dacc52.jpg' border='0' alt=''/></div> <br /><br /> <br /><br /> Фотографии по материалам <i><b>britishpathe.com, lfchistory.net </b></i>",
        ...
        #region blog
        private void InitializeBlogCategories(LiverpoolContext context)
        {
            var type = BlogType;
            var categories = new List<MaterialCategory>()
            {
        ...
        private void InitializeBlog(LiverpoolContext context)
        {
            var type = BlogType;
            var news = new List<Material>();
            var randomizer = new Random(44);
...
<img src= 'http://s3.hostingkartinok.com/uploads/images/2013/07/fc66688ba1ecc06dfe20bc6688dacc52.jpg' border='0' alt=''/></div> <br /><br /> <br /><br /> Фотографии по материалам <i><b>britishpathe.com, lfchistory.net </b></i>",
        ...
        private void InitializeBlogComments(LiverpoolContext context)
        {
            var type = BlogType;
            var newsComments = new List<MaterialComment>();
            var random = new Random((int)DateTime.UtcNow.Ticks);
